import { getModifier, getAttributeValue } from "../utils/helpers";
  ...
  const totalPoints = () =>  10 + 4 * getModifier(getAttributeValue(attributes, "Intelligence"));
  const totalPointsSpent = () => skills.reduce((total, skill) => total + skill.value, 0);

  return (
  <div>
    <h4 style={{marginBottom: 0}}>Skills</h4>
    <h5 style={{marginTop: 0}}>Points Remaining: {totalPoints() - totalPointsSpent() < 0 ? 0 : totalPoints() - totalPointsSpent()}</h5>
